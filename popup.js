const BACKEND_API_URL = "https://youtube-q-a.onrender.com/api/ask"; // <-- Replace with your backend if different
...
document.addEventListener("DOMContentLoaded", function () {
    chrome.tabs.query({ active: true, lastFocusedWindow: true }, function (tabs) {
        ...
        if (tab && tab.url && tab.url.match(/^https:\/\/www\.youtube\.com\/watch\?v=/)) {
            ...
            fetchAnswer(videoUrl, "What is this video about?");
            document.getElementById('ask_btn').disabled = false;
            document.getElementById('question').disabled = false;
            // Save current URL for later - needed for Ask button
            document.getElementById('ask_btn').dataset.videoUrl = videoUrl;
        } else {
            document.getElementById('video_url_group').style.display = "none";
